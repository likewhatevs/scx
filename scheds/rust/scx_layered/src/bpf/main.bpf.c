...
int save_gpu_tgid_pid() {
	...
	pid = pid_tgid >> 32;
	tid = pid_tgid;
	bpf_map_update_elem(&gpu_tid, &tid, 0, BPF_ANY);
	bpf_map_update_elem(&gpu_tgid, &pid, 0, BPF_ANY);
	return 0;
}
...
void BPF_STRUCT_OPS(layered_enqueue, struct task_struct *p, u64 enq_flags)
{
	...
	if(!__COMPAT_is_enq_cpu_selected(enq_flags) || try_preempt_first) {
		if (layer->fifo)
			...
			scx_bpf_dsq_insert_vtime(p, taskc->dsq_id, layer->slice_ns, p->scx.dsq_vtime, enq_flags);
		return;
	}
				 ...
				 struct task_struct *p, const char *cgrp_path)
{
	...
	case MATCH_USED_GPU_TID: {
			...
			if(bpf_map_lookup_elem(&gpu_tgid, &tid) == 0)
				pid_present = true;
