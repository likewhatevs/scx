				 ...
				 struct task_struct *p, const char *cgrp_path)
{
	...
	case MATCH_CGROUP_PREFIX: {
		return match_prefix_suffix(match->cgroup_prefix, cgrp_path, STR_PREFIX);
	}
	case MATCH_CGROUP_SUFFIX: {
		return match_prefix_suffix(match->cgroup_suffix, cgrp_path, STR_SUFFIX);
	}
	case MATCH_CGROUP_CONTAINS: {
		return match_prefix_suffix(match->cgroup_substr, cgrp_path, STR_SUBSTR);
	}
	case MATCH_COMM_PREFIX: {
		char comm[MAX_COMM];
		__builtin_memcpy(comm, p->comm, MAX_COMM);
		return match_prefix_suffix(match->comm_prefix, comm, STR_PREFIX);
	}
	case MATCH_PCOMM_PREFIX: {
		char pcomm[MAX_COMM];

		__builtin_memcpy(pcomm, p->group_leader->comm, MAX_COMM);
		return match_prefix_suffix(match->pcomm_prefix, pcomm, STR_PREFIX);
	}
